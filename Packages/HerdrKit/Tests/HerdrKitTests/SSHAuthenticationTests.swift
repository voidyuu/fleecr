...
final class SSHAuthenticationTests: XCTestCase {
    ...
    func testLegacyLocalCredentialMigratesToKeychain() throws {
        ...
        let query: [String: Any] = [
            ...
            kSecAttrService as String: "cc.cassiel.fleecr.ssh-password",
    ...
    func testLegacyKeychainCredentialMigratesToCurrentService() throws {
        let deviceID = UUID()
        let legacyQuery: [String: Any] = [
            kSecClass as String: kSecClassGenericPassword,
            kSecAttrService as String: "dev.bybee.fleecr.ssh-password",
            kSecAttrAccount as String: deviceID.uuidString,
            kSecValueData as String: Data("legacy-keychain-pass".utf8),
            kSecAttrAccessible as String: kSecAttrAccessibleWhenUnlockedThisDeviceOnly,
        ]
        SecItemDelete(legacyQuery as CFDictionary)
        let addStatus = SecItemAdd(legacyQuery as CFDictionary, nil)
        guard addStatus == errSecSuccess else {
            XCTFail("Failed to add legacy test keychain item: \(addStatus)")
            return
        }
        ...
        XCTAssertEqual(try SSHCredentialStore.password(for: deviceID), "legacy-keychain-pass")
        XCTAssertEqual(SecItemCopyMatching(legacyQuery as CFDictionary, nil), errSecItemNotFound)
        XCTAssertEqual(SecItemCopyMatching(newQuery as CFDictionary, nil), errSecSuccess)
    }
}
final class SSHDestinationTests: XCTestCase {
    func testCustomPortTargetsBecomeSSHURIs() {
        XCTAssertEqual(SSHTunnel.sshDestination("user@10.10.10.87:2222"), "ssh://user@10.10.10.87:2222")
        XCTAssertEqual(SSHTunnel.sshDestination("host.example.com:23"), "ssh://host.example.com:23")
        XCTAssertEqual(SSHTunnel.sshDestination("user@[fe80::1]:2222"), "ssh://user@[fe80::1]:2222")
    }

    func testPlainTargetsPassThroughUntouched() {
        XCTAssertEqual(SSHTunnel.sshDestination("user@10.10.10.87"), "user@10.10.10.87")
        XCTAssertEqual(SSHTunnel.sshDestination("my-config-alias"), "my-config-alias")
        XCTAssertEqual(SSHTunnel.sshDestination("ssh://user@host:2222"), "ssh://user@host:2222")
        // A bare IPv6 address's colons are not a port.
        XCTAssertEqual(SSHTunnel.sshDestination("fe80::1"), "fe80::1")
        XCTAssertEqual(SSHTunnel.sshDestination("user@fe80::1"), "user@fe80::1")
