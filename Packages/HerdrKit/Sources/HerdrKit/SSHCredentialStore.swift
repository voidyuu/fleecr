...
public enum SSHCredentialStore {
    ...
    private static let passwordService = "cc.cassiel.fleecr.ssh-password"
    private static let legacyPasswordServices = [
        "dev.bybee.fleecr.ssh-password",
        "dev.bybee.herdrm.ssh-password",
    ]
    private static let authorizationService = "cc.cassiel.fleecr.ssh-authorization"
    ...
    private static func keychainPassword(for deviceID: UUID) throws -> String? {
        ...
        for legacyService in legacyPasswordServices {
            if let legacyData = try keychainData(
                ...
                let password = try decodePassword(legacyData)
                try setKeychainPassword(password, for: deviceID)
                let legacyQuery = keychainQuery(service: legacyService, account: deviceID.uuidString)
    ...
    private static func removeKeychainPassword(for deviceID: UUID) throws {
        for legacyService in legacyPasswordServices {
            _ = SecItemDelete(
                keychainQuery(service: legacyService, account: deviceID.uuidString) as CFDictionary
            )
        }
        let status = SecItemDelete(
